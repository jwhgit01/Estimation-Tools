function [xarray,ferrorvec,nitervec,iflagtermvec] = epanechnikovSample(n,Ns)
%epanechnikovSample
%
%  Copyright (c) 2009 Mark L. Psiaki.  All rights reserved.  
%
%
%  This function draws Ns samples from an n-dimensional Epanechnikov kernel
%  distribution.  It does so by first sampling a Gaussian distribution and
%  then by rescaling the amplitude based on a sample from a uniform
%  distribution and the solution of an equation involving the cumulative
%  magnitude distribution of the Epanechnikov kernel distribution.  This
%  re-scaling of the amplitude exploits the fact that the Epanechnikov
%  kernel distribution and the zero-mean, identity-covariance Gaussian
%  distribution both produce equi-probable unit direction vectors in
%  n-space.
%
%  Note that this function calls both the randn and the rand random number
%  generators.  Therefore, they must have been appropriately seeded.  This
%  function also makes use of the inverse cumulative distribution function
%  for the Epanechnikov kernel sample magnitude, epanechnikovmagsolve01.m.
%
%  Inputs:
%
...
%
   ...
   for jj = 1:Ns
      [rhojj,ferrorjj,niterjj,iflagtermjj] = epanechnikovMagSolve(probcdfmagvec(1,jj),n);
      rhovec(1,jj) = rhojj;
...
%
   if max(abs(iflagtermvec)) > 0
      disp(' ')
      disp('Warning in epanechnikovSample.m: One or more')
