function [xhat,P] = simpleParticleFilter(t,z,u,f,h,q,Q,R,xhat0,P0,Ns,NT,nRK,params)
...
for k = 0:N-1
    ...
    for ii = 1:Ns
        if isempty(t)
            ...
            zbarikp1 = feval(h,k+1,Xkp1(:,ii),ukp1,0,params);
        else
            tk = t(k,1);
            tkp1 = t(k+1,1);
            Xkp1(:,ii) = c2dNonlinear(Xk(:,ii),uk,Vk(:,ii),tk,tkp1,nRK,f,0,params);
            zbarikp1 = feval(h,tkp1,Xkp1(:,ii),ukp1,0,params);
        end
        nui = zkp1 - zbarikp1;
    ...
    xhat(kp1+1,:) = (Xkp1*wkp1).';
    for ii = 1:Ns
        delxi = Xkp1(:,ii) - xhat(kp1+1,:).';
