function [xhat,yhat,xbar,ybar] = linearStateObserverDT(z,u,F,G,H,D,K,xhat0)
%linearStateObserverDT
%
% Copyright (c) 2023 Jeremy W. Hopwood. All rights reserved.
%
% This function performs linear time invariant state observation for the
% discrete-time system
%
%                       x(k+1) = F*x(k) + G*u(k)                        (1)
%                         y(k) = H*x(k) + D*u(k)                        (2)
%
% given a constant gain matrix K.
%
% Inputs:
%
...
% 
%   F,G,    The system matrices in Eq.(1). If there is no input, G should be
%           an empty array, [].
% 
...
%           state predictions without correction. The index of the estimate is
%           the sample number.
%
%   yhat    The N x ny array that contains the time history of the
%           output estimates.
%
%   ybar    The N x ny array that contains the time history of the
%           output predictions without correction.
%
